...
// ----------------------------------------------------------------------------
// JSON dump helpers (for paper figures / debugging)
// ----------------------------------------------------------------------------

static void json_write_string(std::ostream& out, const std::string& s) {
    out << '"';
    for (const char ch : s) {
        switch (ch) {
            case '\\': out << "\\\\"; break;
...
static void json_write_point(std::ostream& out, const Point& p) {
    out << "[" << p.x() << "," << p.y() << "," << p.z() << "]";
}

static void json_write_int3(std::ostream& out, const int v[3]) {
    out << "[" << v[0] << "," << v[1] << "," << v[2] << "]";
}

static void ensure_parent_dir_exists(const std::filesystem::path& path) {
...
static void dump_site_selection_json(
...
static bool cell_vertices_include_dummy(const Delaunay& dt, const Cell_handle& cell) {
...
static bool cell_ptr_like_vertices_include_dummy(const Delaunay& dt, const CellPtrLike& cell) {
    for (int i = 0; i < 4; ++i) {
        Vertex_handle vh = cell->vertex(i);
        if (dt.is_infinite(vh) || vh->info().is_dummy) {
            return true;
...
static void dump_isosurface_facet_example_json(
    const std::filesystem::path& path,
...
static std::vector<Cell_handle> build_cell_index_vector_local(const Delaunay& dt) {
...
static Cell_handle lookup_cell_local(const std::vector<Cell_handle>& cells, int idx) {
    if (idx < 0 || static_cast<size_t>(idx) >= cells.size()) {
        return Cell_handle();
    }
...
static Vertex_handle find_vertex_by_index(const Delaunay& dt, int idx) {
...
static void dump_multicycle_vertex_example_json(
    const std::filesystem::path& path,
    const VdcParam& param,
    const Delaunay& dt
) {
...
static void dump_isovertex_map_txt(
    const std::filesystem::path& path,
    const DelaunayIsosurface& iso_surface
) {
    if (path.empty()) {
...
int main(int argc, char* argv[]) {
    ...
    if (!param.dump_site_selection_json.empty()) {
        ...
        dump_site_selection_json(param.dump_site_selection_json, grid, param.isovalue, param.sep_dist, param.sep_split, pre, activeCubes);
    }
    ...
    if (!param.dump_facet_example_json.empty()) {
        dump_isosurface_facet_example_json(param.dump_facet_example_json, param, dt);
    }
    ...
    if (!param.dump_multicycle_json.empty()) {
        dump_multicycle_vertex_example_json(param.dump_multicycle_json, param, dt);
    }
    ...
    if (!param.dump_isovertex_map.empty()) {
        dump_isovertex_map_txt(param.dump_isovertex_map, iso_surface);
    }
